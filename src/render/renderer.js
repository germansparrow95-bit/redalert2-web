...
(function (RA) {
  ...
  Renderer.create = function (canvas) {
    ...
    function drawHealthBar(e, playerIdx) {
      ...
      // 敌方血条用深红底衬，一眼区分敌我
      var enemy = (playerIdx === undefined) ? false : (e.owner !== playerIdx);
      var top = e.kind === 'building'
        ? sy - Art.buildingSize(e.type).oy * 0.55 - 10
        : sy - 30;
      var w = e.kind === 'building' ? 34 : 20;
      ctx.fillStyle = enemy ? 'rgba(60,0,0,0.75)' : 'rgba(0,0,0,0.65)';
    ...
    function draw(state, view) {
      ...
      for (i = 0; i < state.buildings.length; i++) {
        ...
        if (Renderer.shouldDrawHealthBar(state, playerIdx, b, view.showHealth)) drawHealthBar(b, playerIdx);
      }
      for (i = 0; i < state.units.length; i++) {
        var u = state.units[i];
        if (Renderer.shouldDrawHealthBar(state, playerIdx, u, view.showHealth)) drawHealthBar(u, playerIdx);
      }
  ...
  /**
   * 是否给这个目标画血条：
   *  · 自己的单位/建筑 —— 受损时显示（满血不画，和原版一致）
   *  · 敌方单位/建筑 —— 需要"显示敌方血条"打开，并且当前真的能看见它
   * （之前这里判断写反了：开关打开时敌方单位反而不显示血条）
   */
  Renderer.shouldDrawHealthBar = function (state, playerIdx, e, showHealth) {
    if (!e || e.dead) return false;
    if (e.owner === playerIdx) return true;
    if (showHealth === false) return false;
    var p = Sim.entityPoint(e);
